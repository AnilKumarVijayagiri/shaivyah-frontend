...
export default function ProductCard({ p }){
  ...
  return (
    <Link to={`/product/${p._id}`} className="card overflow-hidden group">
      <img src={img} alt={p.name} className="w-full h-48 sm:h-56 md:h-64 object-cover transition-transform duration-300 group-hover:scale-105"/>
      <div className="p-3 sm:p-4">
        <h3 className="font-semibold line-clamp-1 text-sm sm:text-base">{p.name}</h3>
        <p className="text-xs sm:text-sm text-gray-500 line-clamp-1">{p.category} · {p.fabric||'Fabric'}</p>
        <div className="mt-1 sm:mt-2 flex items-center gap-2">
          <span className="font-bold text-sm sm:text-base">₹{price}</span>
          {p.discount ? <span className="text-xs line-through text-gray-400">₹{p.price}</span> : null}
        </div>

        {/* Quantity Counter and Add to Cart - Always visible on mobile, hover on desktop */}
        <div className="mt-2 sm:mt-3 flex items-center gap-2 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200" 
             onClick={e => e.preventDefault()}>
          <div className="flex items-center gap-2">
            <button 
              onClick={e => handleQuantityChange(e, quantity - 1)}
